  ...
  return (
    <div>
      <div className="form-grid">
        ...
        <Field label="الحساب" required error={errors.account}>
        ...
        <Field label="الوصف" span={2}>
          <input className="form-control" value={form.description} onChange={e => set('description', e.target.value)} placeholder="وصف العملية (اختياري)" />
        </Field>
        <Field label="رقم المرجع">
          <input className="form-control" value={form.reference} onChange={e => set('reference', e.target.value)} placeholder="رقم المرجع" />
        </Field>
        <Field label="ملاحظات">
          <input className="form-control" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="ملاحظات" />
        </Field>
      </div>
  ...
  return (
    <div className="table-container">
      <table className="table">
        <thead>
          <tr>
            <th>الرقم</th><th>التاريخ</th><th>المبلغ</th><th>الحساب</th>
            <th>الطريقة</th><th>الطرف</th><th>الوصف</th><th>الإجراءات</th>
          </tr>
        </thead>
        <tbody>
          {items.map(item => (
        ...
        <tfoot>
          <tr style={{ background: '#f8fafc' }}>
            ...
            <td className={`number-cell font-bold ${type === 'receipt' ? 'amount-positive' : 'amount-negative'}`}>{formatCurrency(items.reduce((s, i) => s + (parseFloat(i.amount) || 0), 0))}</td>
            <td colSpan={5} />
          </tr>
        </tfoot>
      </table>
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={tab === 'payments' ? `${payments.length} سند • الإجمالي: ${formatCurrency(totalPayments)}` : `${receipts.length} سند • الإجمالي: ${formatCurrency(totalReceipts)}`}
      ...
      <div className="card">
        {tab === 'payments' ? (
          <TxnList items={payments} type="payment" onDelete={handleDelete} accounts={store.accounts} customers={store.customers} suppliers={store.suppliers} employees={store.employees} />
        ) : (
          <TxnList items={receipts} type="receipt" onDelete={handleDelete} accounts={store.accounts} customers={store.customers} suppliers={store.suppliers} employees={store.employees} />
        )}
      </div>

      <Modal open={modal} onClose={() => setModal(false)} title={tab === 'payments' ? 'سند صرف جديد' : 'سند قبض جديد'} size="lg">
        <PaymentForm
          type={tab === 'payments' ? 'payment' : 'receipt'}
          accounts={store.accounts} customers={store.customers}
          suppliers={store.suppliers} employees={store.employees}
